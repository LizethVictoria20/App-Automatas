...
const Header = () => (
  <header className="border-b border-black/10 py-6 px-8 flex justify-between items-center bg-white">
    <div className="flex items-center gap-3">
      ...
      <div>
        <h1 className="font-sans font-medium tracking-tight text-xl">Maquina Turing</h1>
        <p className="text-[10px] uppercase tracking-widest text-black/50 font-mono">Simulador de Autómatas</p>
      </div>
    </div>
    <div className="flex items-center gap-4">
      <div className="flex -space-x-1">
        <div className="w-8 h-8 rounded-full border-2 border-white bg-blue-100 flex items-center justify-center text-[10px] font-bold">LV</div>
        <div className="w-8 h-8 rounded-full border-2 border-white bg-green-100 flex items-center justify-center text-[10px] font-bold">MM</div>
      </div>
    </div>
  </header>
);

const TapeComponent = ({ state, blankSymbol }: { state: TMState, blankSymbol: Symbol }) => {
  const visibleRange = 10;
  const cells = [];
  
  for (let i = state.headIndex - visibleRange; i <= state.headIndex + visibleRange; i++) {
    cells.push({
      index: i,
        ...
        <AnimatePresence initial={false}>
          {cells.map((cell) => (
            ...
            >
              {cell.value}
              {cell.index === state.headIndex && (
                <div className="absolute -top-8 text-[10px] font-bold text-black uppercase tracking-tighter">Cabezal</div>
              )}
            </motion.div>
          ))}
        </AnimatePresence>
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'rules' | 'exercises' | 'config' | 'graph'>('graph');
  ...
  const step = useCallback(async () => {
    ...
    const rule = config.transitions.find(
      t => t.currentState === state.currentState && t.readSymbol === currentSymbol
    );
    ...
    if (!rule) {
      ...
      getExplanation(
        ...
        state.tape, 
        state.headIndex, 
        { currentState: finalState, readSymbol: currentSymbol, writeSymbol: currentSymbol, move: 'N', nextState: finalState }, 
        finalState,
        true
      );
    ...
    const nextStateData: TMState = {
      ...
      isHalted: rule.nextState === 'halt' || rule.nextState === 'accept' || rule.nextState === 'reject',
    };
    ...
    if (nextStateData.isHalted) {
      ...
      getExplanation(state.currentState, state.tape, state.headIndex, rule, nextStateData.currentState, true);
    } else if (activeTab === 'exercises' || Math.random() > 0.8) {
      getExplanation(state.currentState, state.tape, state.headIndex, rule, nextStateData.currentState);
    }
  }, [state, config, activeTab]);

  const getExplanation = async (currState: string, tape: Symbol[], head: number, rule: Transition, nextState: string, isHalt: boolean = false) => {
    try {
  ...
  const toggleRun = () => setState(prev => ({ ...prev, isRunning: !prev.isRunning }));
  ...
  const addTransition = () => {
    setConfig(prev => ({
      ...prev,
      transitions: [...prev.transitions, { currentState: 'q0', readSymbol: '0', writeSymbol: '1', move: 'R', nextState: 'q0' }]
    }));
  };
        ...
        <div className="lg:col-span-8 flex flex-col gap-8">
          ...
          <section className="bg-white p-6 rounded-xl border border-black/5 shadow-sm space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-sm font-medium uppercase tracking-wider text-black/40">Máquina Activa</h2>
                <p className={cn(
                  ...
                  {state.isHalted 
                    ? (state.currentState === 'accept' ? 'Cadena Aceptada ✓' : 
                       state.currentState === 'reject' ? 'Cadena Rechazada ✗' : 'Simulación Finalizada') 
                    : 'Ejecución del Simulador'}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-mono text-black/40 uppercase">Estado</span>
                  <span className="font-mono font-medium px-3 py-1 bg-black text-white rounded-sm">{state.currentState}</span>
                </div>
                <div className="flex flex-col items-end border-l border-black/10 pl-4 ml-2">
                  <span className="text-[10px] font-mono text-black/40 uppercase">Pasos</span>
                  <span className="font-mono font-medium">{state.stepCount}</span>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-xl border border-black/5 shadow-md overflow-hidden relative">
            ...
            <div className="px-8 py-6 flex items-center justify-between bg-white border-t border-black/5">
              <div className="flex items-center gap-3">
                ...
                >
                  {state.isRunning ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" className="ml-1" />}
                </button>
                <button 
                  onClick={step}
              ...
              <div className="flex items-center gap-4">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase font-bold text-black/30 mb-1">Velocidad</span>
                  <input 
                ...
                <div className="text-xs font-mono w-12 text-right">{speed}ms</div>
              </div>
            </div>
          </section>
                ...
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === tab.id ? "bg-black text-white" : "hover:bg-black/5 text-black/40"
                    )}
                  >
                    {tab.label}
                  </button>
                  ...
                  <div className="h-full flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-black/40 uppercase font-mono">
                        ...
                        <div className="ml-2 w-2 h-2 rounded-full border border-black border-dashed"></div> Inicio
                      </div>
                    </div>
                ...
                {activeTab === 'rules' && (
                  <div className="space-y-4">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button onClick={addTransition} className="flex items-center gap-2 text-[10px] font-bold uppercase py-2 px-4 border border-black/10 rounded-full hover:bg-black hover:text-white transition-colors">
                    ...
                    <table className="w-full text-left text-sm border-collapse">
                      ...
                      <tbody className="font-mono">
                        {config.transitions.map((t, i) => (
                          <tr key={i} className="group border-b border-black/5 hover:bg-black/5 transition-colors">
                            <td className="py-2 px-1">
                              <input 
                                ...
                                onChange={(e) => updateTransition(i, 'currentState', e.target.value)} 
                              />
                            </td>
                            <td className="py-2 px-1">
                              <input 
                                ...
                                value={t.readSymbol} 
                                onChange={(e) => updateTransition(i, 'readSymbol', e.target.value)} 
                              />
                            </td>
                            <td className="py-2 px-1">
                              <input 
                                className="w-8 bg-transparent outline-none focus:bg-white focus:ring-1 focus:ring-black text-center border-x border-black/5" 
                                value={t.writeSymbol} 
                                onChange={(e) => updateTransition(i, 'writeSymbol', e.target.value)} 
                              />
                            </td>
                            <td className="py-2 px-1">
                              <select 
                                ...
                                value={t.move} 
                                onChange={(e) => updateTransition(i, 'move', e.target.value as any)}
                              >
                            ...
                            <td className="py-2 px-1">
                              <input 
                                ...
                                value={t.nextState} 
                                onChange={(e) => updateTransition(i, 'nextState', e.target.value)} 
                              />
                            </td>
                            <td className="py-2 px-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => removeTransition(i)} className="text-red-500 hover:text-red-700">
                                <Trash2 size={14} />
                              </button>
                            </td>
                          </tr>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {[{ name: 'Incremento Binario', id: 'binaryIncrement' }, { name: 'Verificador de Palíndromos', id: 'palindrome' }, { name: 'Suma Unaria', id: 'unary' }].map((ex, i) => (
                        ...
                        >
                          ...
                          <ChevronRight className="opacity-0 group-hover:opacity-100 translate-x-[-10px] group-hover:translate-x-0 transition-all" />
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {activeTab === 'config' && (
                  <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-[10px] font-bold uppercase opacity-40">Contenido Inicial de la Cinta</label>
                       <input 
                         className="w-full p-4 bg-black/5 rounded-lg font-mono outline-none focus:ring-2 focus:ring-black"
                         value={config.tape.join('')}
                         onChange={(e) => setConfig(prev => ({ ...prev, tape: e.target.value.split('') }))}
                       />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[10px] font-bold uppercase opacity-40">Símbolo Blanco (Blank)</label>
                       <input 
                         className="w-full p-4 bg-black/5 rounded-lg font-mono outline-none focus:ring-2 focus:ring-black"
                         value={config.blankSymbol}
                         onChange={(e) => setConfig(prev => ({ ...prev, blankSymbol: e.target.value }))}
                       />
                    </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          ...
          <section className="bg-black text-white p-8 rounded-xl shadow-2xl flex flex-col gap-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xs font-bold uppercase tracking-[0.2em]">Explicación del Paso</h3>
            </div>
            ...
            <div className="min-h-[120px] flex items-center">
              {isExplaining ? (
                ...
                <p className="text-white/30 italic text-sm font-serif">Ejecuta la máquina para ver la lógica paso a paso explicada por la IA de Turing...</p>
              )}
            </div>

            <div className="pt-6 border-t border-white/10 flex justify-between items-center text-[10px]">
              <span className="text-white/40 uppercase font-mono tracking-widest">Estado: Lista</span>
          ...
          <section className="bg-white p-8 rounded-xl border border-black/5 shadow-sm space-y-6">
            <div className="flex items-center gap-3">
              <BookOpen size={18} />
              <h3 className="text-xs font-bold uppercase tracking-[0.2em]">Nodo de Aprendizaje</h3>
            </div>
